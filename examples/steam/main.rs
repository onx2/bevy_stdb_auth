...
fn main() {
    App::new()
        ...
        .add_systems(Startup, start_steam_login)
        .add_systems(Update, (observe_auth_messages, exit_on_timeout))
        .run();
}

fn start_steam_login(mut auth: StdbAuthCommands, mut exit: MessageWriter<AppExit>) {
    ...
    let app_id = env::var("STEAM_APP_ID")
        .ok()
        .and_then(|app_id| app_id.parse::<u32>().ok())
        .unwrap_or(DEFAULT_STEAM_APP_ID);
    let options = StdbSteamAuthOptions { client_id, app_id };

    if let Err(error) = auth.login(StdbLoginOptions::new(StdbAuthSource::Steam(options))) {
